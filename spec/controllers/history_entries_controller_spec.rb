...
describe HistoryEntriesController do
  ...
  def mock_history_entry(stubs={})
    @mock_history_entry ||= mock_model(HistoryEntry, stubs)
  end

  describe "GET index" do
    it "assigns all history_entries as @history_entries" do
      HistoryEntry.stub(:find).with(:all).and_return([mock_history_entry])
      get :index
      assigns[:history_entries].should == [mock_history_entry]
    end
  end

  describe "GET show" do
    it "assigns the requested history_entry as @history_entry" do
      HistoryEntry.stub(:find).with("37").and_return(mock_history_entry)
      get :show, :id => "37"
      assigns[:history_entry].should equal(mock_history_entry)
    end
  end

  describe "GET new" do
    it "assigns a new history_entry as @history_entry" do
      HistoryEntry.stub(:new).and_return(mock_history_entry)
      get :new
      assigns[:history_entry].should equal(mock_history_entry)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created history_entry as @history_entry" do
        HistoryEntry.stub(:new).with({'these' => 'params'}).and_return(mock_history_entry(:save => true))
        post :create, :history_entry => {:these => 'params'}
        assigns[:history_entry].should equal(mock_history_entry)
      end

      it "redirects to the created history_entry" do
        HistoryEntry.stub(:new).and_return(mock_history_entry(:save => true))
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved history_entry as @history_entry" do
        HistoryEntry.stub(:new).with({'these' => 'params'}).and_return(mock_history_entry(:save => false))
        post :create, :history_entry => {:these => 'params'}
        assigns[:history_entry].should equal(mock_history_entry)
      end

      it "re-renders the 'new' template" do
        HistoryEntry.stub(:new).and_return(mock_history_entry(:save => false))
        post :create, :history_entry => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested history_entry" do
        HistoryEntry.should_receive(:find).with("37").and_return(mock_history_entry)
        mock_history_entry.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :history_entry => {:these => 'params'}
      end

      it "assigns the requested history_entry as @history_entry" do
        HistoryEntry.stub(:find).and_return(mock_history_entry(:update_attributes => true))
        put :update, :id => "1"
        assigns[:history_entry].should equal(mock_history_entry)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the history_entry as @history_entry" do
        HistoryEntry.stub(:find).and_return(mock_history_entry(:update_attributes => false))
        put :update, :id => "1"
        assigns[:history_entry].should equal(mock_history_entry)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the history_entries list" do
      HistoryEntry.stub(:find).and_return(mock_history_entry(:destroy => true))
      delete :destroy, :id => "1"
